...
if($totalRows_contra>=1){
  ...
  $Result1 = mysql_query($updateSQL, $funeraria) or die(mysql_error());
      ...
      if($Result1){
  echo "<script type=\"text/javascript\">alert ('Datos Modificados');  location.href='consulta_contrato_cedula2.php?cedula=$_POST[cliente]' </script>";
  }else{
  echo "<script type=\"text/javascript\">alert ('Ocurrio un Error');  location.href='consulta_contrato_cedula2.php?cedula=$_POST[cliente]' </script>";
  exit;
  }
}
...
do {  
?>
          <option value="<?php echo $row_clientes['cedula']?>"<?php if (!(strcmp($row_clientes['cedula'], $row_contratos['cliente']))) {echo "selected=\"selected\"";} ?>><?php echo $row_clientes['nombres']?></option>
          <?php
...
?>
    ...
    <tr>
      ...
      <td><input name="numero" type="text" id="numero" value="<?php echo $row_contratos['numero']; ?>" maxlength="3" /></td>
    </tr>
    <tr>
